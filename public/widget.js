/* Ryde Dental Family chat widget — paste ONE script tag on the site:
   ...
   Talks to the backend it is served from. No build step needed. */
(function () {
  var API = (function () {
    try { var u = new URL(document.currentScript.src); return u.origin; } catch (e) { return ""; }
  })();
  ...
  var seen = {}; msgs.forEach(function (m) { if (m.ts) seen[m.ts] = 1; });
  var mode = "ai", open = false, listening = false, recog = null, pollTimer = null;
  var COLD_MS = 3500, warmTimer = null;

  var C = { teal: "#0E5A57", tealDeep: "#0A3F3D", coral: "#F2615A", coralDeep: "#DC4F49",
    ink: "#0F2E2E", mint: "#EAF4F2", line: "#DCEAE7", muted: "#5B7472", bg: "#EDF3F1" };
  ...
  var css = "" +
    ...
    "#rdf-head .st{color:#BFE6E2;font-size:12px;display:flex;align-items:center;gap:6px}" +
    "#rdf-head.human{background:linear-gradient(135deg," + C.coralDeep + ",#B83F3A)}#rdf-head.human .st{color:#FFE3D8}" +
    "#rdf-x{margin-left:6px;background:none;border:none;color:#fff;cursor:pointer;font-size:20px;opacity:.85}" +
    "#rdf-body{flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px}" +
    ...
    ".rdf-b{max-width:80%;padding:10px 13px;font-size:14px;line-height:1.5;white-space:pre-wrap}" +
    ...
    ".rdf-b.team{background:#FFEDEB;color:" + C.ink + ";border:1px solid #F6D3D0;border-top:2px solid " + C.coral + ";border-radius:16px 16px 16px 4px}" +
    ".rdf-team-l{font-size:11px;font-weight:700;color:" + C.coralDeep + ";margin:0 0 3px 2px}" +
    ".rdf-sys{align-self:center;font-size:11.5px;color:" + C.muted + ";background:#E7EEEC;padding:4px 11px;border-radius:20px}" +
    ".rdf-av{width:26px;height:26px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:linear-gradient(145deg,#157E79," + C.tealDeep + ")}" +
    ...
    "#rdf-in{flex:1;border:none;outline:none;padding:11px 0;font-size:14px;background:transparent;color:" + C.ink + "}" +
    ...
    "#rdf-form{position:absolute;left:0;right:0;top:62px;bottom:0;background:#F4FAF8;z-index:5;display:flex;flex-direction:column;animation:rdfin .2s ease}" +
    ".rdf-fh{padding:13px 16px;font-weight:700;font-size:14px;background:#fff;border-bottom:1px solid " + C.line + ";display:flex;align-items:center}" +
    ...
    ".rdf-fi{padding:11px 12px;border:1px solid " + C.line + ";border-radius:10px;outline:none;font-size:14px;background:#fff;color:" + C.ink + "}" +
    ...
    "#rdf-warm{background:#fff;border:1px solid " + C.line + ";border-radius:14px;padding:14px;display:flex;flex-direction:column;gap:8px;animation:rdfin .25s ease}" +
    ".rdf-wt{font-weight:700;font-size:13.5px;color:" + C.ink + ";display:flex;align-items:center;gap:7px}" +
    ".rdf-ws{font-size:12.5px;color:" + C.muted + ";margin:-2px 0 2px}" +
    ".rdf-spin{width:14px;height:14px;border:2px solid " + C.line + ";border-top-color:" + C.teal + ";border-radius:50%;display:inline-block;animation:rdfspin .8s linear infinite}" +
    "@keyframes rdfspin{to{transform:rotate(360deg)}}" +
    ".rdf-dot{width:6px;height:6px;border-radius:50%;background:#157E79;display:inline-block;animation:rdfd 1s infinite}" +
  ...
  var micI = '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#5B7472" stroke-width="2"><rect x="9" y="2" width="6" height="11" rx="3"/><path d="M5 10a7 7 0 0014 0M12 17v4"/></svg>';
  var clipI = '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#5B7472" stroke-width="2"><path d="M21 12.5l-8.5 8.5a5 5 0 01-7-7l9-9a3.5 3.5 0 015 5l-9 9a2 2 0 01-3-3l8.5-8.5"/></svg>';
  ...
  root.innerHTML =
    ...
    '<div id="rdf-panel">' +
      '<div id="rdf-head"><div class="av">' + spark + '</div><div><div class="nm">Ryde Dental Family</div>' +
        '<div class="st"><span id="rdf-dot2" style="width:7px;height:7px;border-radius:50%;background:#5FE3B0;display:inline-block"></span><span id="rdf-stt">Smily · replies instantly</span></div></div>' +
        '<button id="rdf-book" title="Book appointment" style="margin-left:auto;background:none;border:none;cursor:pointer;padding:4px;display:flex;align-items:center">' + '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>' + '</button><button id="rdf-x" aria-label="Close">&times;</button></div>' +
      '<div id="rdf-body"></div>' +
  ...
  function render() {
    ...
    msgs.forEach(function (m) {
      if (m.role === "system") { body.appendChild(el("div", "rdf-sys", esc(m.text))); return; }
  ...
  function renderChips() {
    ...
    last.chips.forEach(function (c) {
      var b = el("button", "rdf-chip", esc(c)); b.onclick = function () { sendMsg(c); }; chipsEl.appendChild(b);
    });
  }
  function typing(on) {
  ...
  function setMode(m) {
    ...
    $("rdf-dot2").style.background = m === "human" ? "#FFD7A8" : "#5FE3B0";
    $("rdf-head").querySelector(".av").innerHTML = m === "human" ? head : spark;
  }

  function push(role, text, extra) { var m = Object.assign({ role: role, text: text, ts: Date.now() + Math.random() }, extra || {}); msgs.push(m); seen[m.ts] = 1; persist(); render(); }
  ...
  async function sendMsg(text) {
    ...
    chipsEl.innerHTML = "";
    typing(true);
    warmTimer = setTimeout(showWarmCapture, COLD_MS);
    try {
      ...
      var d = await r.json();
      clearTimeout(warmTimer); typing(false); removeWarm();
      if (d.mode) setMode(d.mode);
      if (d.reply) push("bot", d.reply, { chips: d.chips || [] });
      else if (d.queued) { /* a human is handling — their reply arrives via poll */ }
    } catch (e) { clearTimeout(warmTimer); typing(false); push("bot", "Sorry, I couldn't reach the clinic just now — please call (02) 9807 9800.", { chips: [] }); }
  }
  ...
  async function poll() {
    try {
      ...
      (d.events || []).forEach(function (ev) {
        if (seen[ev.ts]) return; seen[ev.ts] = 1;
        msgs.push({ role: ev.role, text: ev.text, ts: ev.ts }); persist();
      });
      render();
    } catch (e) {}
  }
  ...
  function removeWarm(){ var w = $("rdf-warm"); if (w && !localStorage.getItem("rdf_captured")) w.remove(); }
  // While the (free) server wakes up, collect details so the lead is never lost
  function showWarmCapture(){
    if ($("rdf-warm") || localStorage.getItem("rdf_captured")) return;
    var c = el("div"); c.id = "rdf-warm";
    c.innerHTML = '<div class="rdf-wt"><span class="rdf-spin"></span> Connecting you to Smily…</div>' +
      '<div class="rdf-ws">Takes a few seconds. Pop your details here so the team can reach you if needed:</div>' +
      '<input class="rdf-fi" id="wm-name" placeholder="Name"/>' +
      '<input class="rdf-fi" id="wm-phone" placeholder="Mobile"/>' +
      '<input class="rdf-fi" id="wm-email" placeholder="Email"/>' +
      '<button id="wm-send" class="rdf-fbtn">Save my details</button>';
    body.appendChild(c); body.scrollTop = body.scrollHeight;
    $("wm-send").onclick = function(){
      var name = $("wm-name").value.trim(), phone = $("wm-phone").value.trim(), email = $("wm-email").value.trim();
      if (!name || !phone){ $("wm-send").textContent = "Add your name & mobile"; return; }
      $("wm-send").textContent = "Saving…"; $("wm-send").disabled = true;
      localStorage.setItem("rdf_captured", "1");
      fetch(API + "/api/book", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: SID, name: name, phone: phone, email: email, service: "Website enquiry" }) })
        .then(function(){ var x = $("rdf-warm"); if (x) x.innerHTML = '<div class="rdf-wt">✅ Thanks ' + esc(name.split(" ")[0]) + '!</div><div class="rdf-ws">The team can reach you on ' + esc(phone) + '. Smily is almost ready…</div>'; })
        .catch(function(){ $("wm-send").textContent = "Try again"; $("wm-send").disabled = false; localStorage.removeItem("rdf_captured"); });
    };
  }
  ...
  $("rdf-file").onchange = function (e) {
    var f = e.target.files[0]; if (!f) return; e.target.value = "";
    push("user", "📎 " + f.name);
    fetch(API + "/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: SID, attachment: f.name }) })
      .then(function (r) { return r.json(); }).then(function (d) { if (d.reply) push("bot", d.reply, { chips: d.chips || [] }); }).catch(function () {});
  };
})();
